...
/*
 * taro.c - Thin C shim over the QuickJS C API
 *
 * All JSValue* returned are malloc'd with malloc(), freed with taro_free_value().
...
const char* taro_last_error(void) {
    return _taro_last_error[0] ? _taro_last_error : NULL;
}

void taro_clear_error(void) {
    _taro_last_error[0] = 0;
}

/* Heap-allocate a JSValue and return it as void* */
static void* _taro_alloc_value(JSValue val) {
...
static char* _module_normalize(JSContext* ctx, const char* base_name,
    ...
    if (strcmp(name, "milktea") == 0 || strcmp(name, "os") == 0 || strcmp(name, "fs") == 0)
...
static JSModuleDef* _module_loader(JSContext* ctx, const char* name, void* opaque) {
    ...
    /* ---- "milktea" virtual module ---- */
    if (strcmp(name, "milktea") == 0) {
        JSValue func = JS_Eval(ctx, _milktea_module_src, strlen(_milktea_module_src),
                               "milktea", JS_EVAL_TYPE_MODULE | JS_EVAL_FLAG_COMPILE_ONLY);
...
void* taro_new_runtime(void) {
    return JS_NewRuntime();
}

void* taro_new_context(void* rt) {
    if (!rt) return NULL;
...
void taro_free(void* rt, void* ctx) {
    if (ctx) {
...
int taro_eval(void* ctx, const char* src, size_t len, const char* filename) {
    if (!ctx || !src) return -1;
    ...
    if (JS_IsException(result)) {
        ...
        if (exc_str) {
            fprintf(stderr, "[taro] eval error: %s\n", exc_str);
...
int taro_eval_module(void* rt, void* ctx, const char* src, size_t len, const char* filename) {
    if (!ctx || !src) return -1;
    JSContext* c = (JSContext*)ctx;
    ...
    if (JS_IsException(func)) {
        ...
        if (s) { fprintf(stderr, "[taro] module compile error: %s\n", s); JS_FreeCString(c, s); }
        JS_FreeValue(c, exc);
        return -1;
    }
    ...
    if (JS_IsException(result)) {
        JSValue exc = JS_GetException(c);
        const char* s = JS_ToCString(c, exc);
        if (s) { fprintf(stderr, "[taro] module eval error: %s\n", s); JS_FreeCString(c, s); }
        JS_FreeValue(c, exc);
        JS_FreeValue(c, result);
        return -1;
    }
...
void* taro_get_global(void* ctx) {
    if (!ctx) return NULL;

    JSValue global = JS_GetGlobalObject((JSContext*)ctx);
    return _taro_alloc_value(global);
}

void* taro_get_prop(void* ctx, void* obj, const char* name) {
    if (!ctx || !obj || !name) return NULL;

    JSValue obj_val = *(JSValue*)obj;
    JSValue prop_val = JS_GetPropertyStr((JSContext*)ctx, obj_val, name);

    return _taro_alloc_value(prop_val);
}

int taro_call(void* ctx, void* fn_val, void* this_val, int argc, void** argv, void** out) {
    ...
    if (JS_IsException(result)) {
        ...
        if (stack_str)
            snprintf(_taro_last_error, sizeof(_taro_last_error), "%s", stack_str);
        else if (exc_str)
            snprintf(_taro_last_error, sizeof(_taro_last_error), "%s", exc_str);

        /* Also print to stderr */
        if (exc_str) fprintf(stderr, "\r[taro] JS error: %s\r\n", exc_str);
        if (stack_str) {
...
void* taro_new_object(void* ctx) {
    if (!ctx) return NULL;

    JSValue obj = JS_NewObject((JSContext*)ctx);
    return _taro_alloc_value(obj);
}

void* taro_new_int32(void* ctx, int val) {
    if (!ctx) return NULL;
    return _taro_alloc_value(JS_NewInt32((JSContext*)ctx, val));
}

void taro_set_prop_str(void* ctx, void* obj, const char* name, const char* val) {
...
void taro_set_prop_str_n(void* ctx, void* obj, const char* name, const char* val, size_t len) {
    if (!ctx || !obj || !name || !val) return;
...
void taro_set_prop_int(void* ctx, void* obj, const char* name, int val) {
...
void taro_set_prop_bool(void* ctx, void* obj, const char* name, int val) {
    if (!ctx || !obj || !name) return;
...
void taro_set_prop_obj(void* ctx, void* obj, const char* name, void* child) {
...
char* taro_to_cstring(void* ctx, void* val) {
...
void taro_free_cstring(void* ctx, char* str) {
    (void)ctx;  /* unused */
    if (str) {
        free(str);
    }
...
int taro_is_null(void* ctx, void* val) {
    (void)ctx;  /* unused */
    if (!val) return 1;
...
int taro_is_function(void* ctx, void* val) {
    if (!ctx || !val) return 0;
...
void taro_free_value(void* ctx, void* val) {
    if (!val) return;
...
typedef struct {
    taro_cfunc_t fn;
} taro_cfunc_wrapper_t;

static JSValue _taro_cfunc_trampoline(JSContext* ctx, JSValueConst this_val,
    ...
    taro_cfunc_wrapper_t* wrapper = (taro_cfunc_wrapper_t*)JS_GetOpaque(func_data[0], 0);
    if (!wrapper) {
...
void taro_set_global_fn(void* ctx, const char* name, taro_cfunc_t fn_ptr, int length) {
    if (!ctx || !name || !fn_ptr) return;

    JSContext* c = (JSContext*)ctx;

    /* Create a wrapper to store the function pointer */
    taro_cfunc_wrapper_t* wrapper = (taro_cfunc_wrapper_t*)malloc(sizeof(taro_cfunc_wrapper_t));
    if (!wrapper) return;
    ...
    JSValue cfunc = JS_NewCFunctionData(c, _taro_cfunc_trampoline, length, 0, 1, &wrapper_obj);
...
int taro_is_string(void* ctx, void* val) {
    (void)ctx;
    if (!val) return 0;
    JSValue js_val = *(JSValue*)val;
...
int taro_is_array(void* ctx, void* val) {
    if (!ctx || !val) return 0;
    JSValue js_val = *(JSValue*)val;
    return JS_IsArray((JSContext*)ctx, js_val);
}

int taro_array_length(void* ctx, void* val) {
    if (!ctx || !val) return 0;
    JSContext* c = (JSContext*)ctx;
    JSValue js_val = *(JSValue*)val;
...
void* taro_array_item(void* ctx, void* val, int idx) {
    if (!ctx || !val) return NULL;
    JSContext* c = (JSContext*)ctx;
    JSValue js_val = *(JSValue*)val;
    JSValue item = JS_GetPropertyUint32(c, js_val, (uint32_t)idx);
    return _taro_alloc_value(item);
}
...
void taro_execute_pending_jobs(void* rt) {
    if (!rt) return;
    JSContext* pctx;
    int ret;
    do {
        ...
        if (ret < 0 && pctx) {
            ...
            if (s) {
                fprintf(stderr, "[taro] unhandled promise rejection: %s\n", s);
...
static taro_timer_t _timers[TARO_MAX_TIMERS];
...
static void _timers_ensure_init(void) {
    ...
    for (int i = 0; i < TARO_MAX_TIMERS; i++) {
        _timers[i].callback = JS_UNDEFINED;
    }
    _timers_init = 1;
}

int taro_set_timeout(void* ctx, void* callback_val, long long delay_ms) {
    if (!ctx || !callback_val) return -1;
    _timers_ensure_init();

    JSContext* c = (JSContext*)ctx;
    long long now = taro_now_ms();
    ...
    fprintf(stderr, "[taro] timer queue full\n");
    return -1;
}

void taro_clear_timeout(void* ctx, int timer_id) {
...
void taro_run_timers(void* rt, void* ctx) {
    if (!rt || !ctx) return;
    _timers_ensure_init();
    JSContext* c = (JSContext*)ctx;
    long long now = taro_now_ms();
    ...
    for (int i = 0; i < TARO_MAX_TIMERS; i++) {
        if (_timers[i].active && _timers[i].expiry_ms <= now) {
            ...
            if (JS_IsException(result)) {
                ...
                const char* s = JS_ToCString(c, exc);
                if (s) {
                    fprintf(stderr, "[taro] timer callback error: %s\n", s);
                    JS_FreeCString(c, s);
                }
                JS_FreeValue(c, exc);
            }
...
typedef struct taro_chunk {
    int      req_id;    /* matches taro_fetch_req_t.req_id */
    char*    text;      /* strdup'd chunk text, NULL = stream done */
    int      done;      /* 1 = stream ended (text may be NULL) */
    struct taro_chunk* next;
} taro_chunk_t;

static pthread_mutex_t _chunk_mutex = PTHREAD_MUTEX_INITIALIZER;
static taro_chunk_t*   _chunk_head  = NULL;
static taro_chunk_t*   _chunk_tail  = NULL;

static void _chunk_push(taro_chunk_t* c) {
...
static taro_chunk_t* _chunk_pop(void) {
    pthread_mutex_lock(&_chunk_mutex);
    taro_chunk_t* c = _chunk_head;
    if (c) {
        _chunk_head = c->next;
...
#define TARO_MAX_STREAMS 16
typedef struct {
    int  req_id;
    int  active;
} taro_stream_slot_t;

static taro_stream_slot_t _streams[TARO_MAX_STREAMS];
...
static int _stream_alloc(void) {
    ...
    for (int i = 0; i < TARO_MAX_STREAMS; i++) {
...
typedef struct taro_fetch_resp {
    JSValue  resolve;
    JSValue  reject;
    char*    body;
    size_t   body_len;
    long     status;
    char*    error_msg;
    struct taro_fetch_resp* next;
} taro_fetch_resp_t;

static pthread_mutex_t _fetch_mutex = PTHREAD_MUTEX_INITIALIZER;
static taro_fetch_resp_t* _fetch_resp_head = NULL;
static taro_fetch_resp_t* _fetch_resp_tail = NULL;

int taro_has_pending_fetch(void) {
...
static void _fetch_push_resp(taro_fetch_resp_t* resp) {
    pthread_mutex_lock(&_fetch_mutex);
    resp->next = NULL;
...
static taro_fetch_resp_t* _fetch_pop_resp(void) {
    pthread_mutex_lock(&_fetch_mutex);
    taro_fetch_resp_t* r = _fetch_resp_head;
    if (r) {
            ...
            *p = '\0';
            ...
            if (strncmp(line_start, "data: ", 6) == 0) {
                ...
                if (strcmp(payload, "[DONE]") != 0 && strlen(payload) > 0) {
                    taro_chunk_t* chunk = (taro_chunk_t*)calloc(1, sizeof(taro_chunk_t));
                    if (chunk) {
...
/* ---- fetch thread ---- */
static void _setup_curl_common(CURL* curl, taro_fetch_req_t* req,
                               struct curl_slist** hdrs_out) {
    curl_easy_setopt(curl, CURLOPT_URL,            req->url);
    curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1L);
    curl_easy_setopt(curl, CURLOPT_TIMEOUT,        60L);
    curl_easy_setopt(curl, CURLOPT_USERAGENT,      "taro/1.0");
...
static void* _fetch_thread(void* arg) {
    taro_fetch_req_t* req = (taro_fetch_req_t*)arg;

    CURL* curl = curl_easy_init();
    if (!curl) {
        if (req->streaming) {
            taro_chunk_t* done = (taro_chunk_t*)calloc(1, sizeof(taro_chunk_t));
            if (done) { done->req_id = req->req_id; done->done = 1;
                        done->text = strdup("fetch: curl_easy_init failed"); _chunk_push(done); }
        } else {
            taro_fetch_resp_t* resp = (taro_fetch_resp_t*)calloc(1, sizeof(taro_fetch_resp_t));
    ...
    if (req->streaming) {
        ...
        taro_chunk_t* sentinel = (taro_chunk_t*)calloc(1, sizeof(taro_chunk_t));
        if (sentinel) {
...
void taro_drain_fetch_responses(void* rt, void* ctx) {
    ...
    while ((chunk = _chunk_pop()) != NULL) {
        if (chunk->done) {
            /* __taro_stream_done(req_id, error_or_null) */
            JSValue fn = JS_GetPropertyStr(c, global, "__taro_stream_done");
            ...
            /* __taro_push_chunk(req_id, text) */
            JSValue fn = JS_GetPropertyStr(c, global, "__taro_push_chunk");
            if (JS_IsFunction(c, fn)) {
                JSValue args[2];
                args[0] = JS_NewInt32(c, chunk->req_id);
    ...
    while ((resp = _fetch_pop_resp()) != NULL) {
        if (resp->error_msg) {
            ...
            /* Attach text()/json() via pre-compiled __taro_make_response from milktea.js */
            JSValue global = JS_GetGlobalObject(c);
            JSValue make_resp_fn = JS_GetPropertyStr(c, global, "__taro_make_response");
            JS_FreeValue(c, global);
...
static JSValue _js_set_timeout(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    /* taro_set_timeout expects void* pointing to a JSValue; it dups internally */
    int id = taro_set_timeout(ctx, (void*)&argv[0], delay);
...
static JSValue _js_fetch(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    taro_fetch_req_t* req = (taro_fetch_req_t*)calloc(1, sizeof(taro_fetch_req_t));
    if (!req) {
        JS_FreeCString(ctx, url_cstr);
    ...
    /* For streaming requests: call __taro_stream_alloc(req_id) in JS, then
    ...
    if (req->streaming && req->req_id >= 0) {
        ...
        /* __taro_stream_alloc(req_id) — sets up the JS-side push queue */
        JSValue alloc_fn = JS_GetPropertyStr(ctx, global, "__taro_stream_alloc");
        ...
        /* __taro_make_stream_response(req_id, 0) — returns response object with .body */
        JSValue make_fn = JS_GetPropertyStr(ctx, global, "__taro_make_stream_response");
...
/* taro_set_timeout wrapper for void* callback (used by existing API) */
int taro_set_timeout_raw(void* ctx, JSValue cb, long long delay_ms) {
    JSValue* ptr = (JSValue*)malloc(sizeof(JSValue));
    if (!ptr) return -1;
    *ptr = cb;  /* already dup'd by caller */
    int id = taro_set_timeout(ctx, ptr, delay_ms);
    free(ptr);  /* taro_set_timeout dup'd it internally */
    return id;
}
...
#define TARO_MAX_INTERVALS 64
static taro_interval_t _intervals[TARO_MAX_INTERVALS];
...
static void _interval_fire_cb(JSContext* ctx, taro_interval_t* iv);
...
/* We implement setInterval in JS-land via milktea.js using setTimeout recursion —
...
static JSValue _js_set_interval(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    /* setInterval(fn, delay) — implemented as recursive setTimeout in milktea.js;
...
extern void          taro__textarea_handle_key_ptr(void* ta_ptr, int code, int rune, int ctrl, int alt, int shift);
extern void          taro__textarea_clear_ptr(void* ta_ptr);
extern int           taro__textarea_cursor_row_ptr(void* ta_ptr);
extern int           taro__textarea_cursor_col_ptr(void* ta_ptr);
extern const char*   taro__textarea_text_ptr(void* ta_ptr);
extern int           taro__textarea_text_len(void* ta_ptr);

extern void*         taro__viewport_get_ptr(const char* id);
extern void          taro__viewport_handle_key_ptr(void* vp_ptr, int code, int ctrl, int alt, int shift);
extern void          taro__viewport_scroll_to_bottom_ptr(void* vp_ptr);
extern int           taro__viewport_offset_ptr(void* vp_ptr);
extern void          taro__viewport_scroll_up_ptr(void* vp_ptr);
extern void          taro__viewport_scroll_down_ptr(void* vp_ptr);
extern void          taro__viewport_page_up_ptr(void* vp_ptr);
extern void          taro__viewport_page_down_ptr(void* vp_ptr);
...
static JSValue _js_textarea_update(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    void* ta_ptr = taro__textarea_get_ptr(id);
    ...
    taro__textarea_handle_key_ptr(ta_ptr, code, rune, ctrl, alt, shift);
    return JS_UNDEFINED;
}
...
static JSValue _js_textarea_get_text(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    int len = taro__textarea_text_len(ta_ptr);
    if (len <= 0) return JS_NewString(ctx, "");
    const char* text = taro__textarea_text_ptr(ta_ptr);
...
static JSValue _js_textarea_clear(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    void* ta_ptr = taro__textarea_get_ptr(id);
    JS_FreeCString(ctx, id);
    if (ta_ptr) taro__textarea_clear_ptr(ta_ptr);
    return JS_UNDEFINED;
}
...
static JSValue _js_viewport_update(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    const char* id = JS_ToCString(ctx, argv[0]);
    if (!id) return JS_EXCEPTION;
    void* vp_ptr = taro__viewport_get_ptr(id);
    JS_FreeCString(ctx, id);
    ...
    taro__viewport_handle_key_ptr(vp_ptr, code, ctrl, alt, shift);
    return JS_UNDEFINED;
}
...
static JSValue _js_viewport_scroll_to_bottom(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    void* vp_ptr = taro__viewport_get_ptr(id);
    JS_FreeCString(ctx, id);
    if (vp_ptr) taro__viewport_scroll_to_bottom_ptr(vp_ptr);
    return JS_UNDEFINED;
}
...
static JSValue _js_textarea_get_cursor(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    void* ta_ptr = id ? taro__textarea_get_ptr(id) : NULL;
    if (id) JS_FreeCString(ctx, id);
    int row = ta_ptr ? taro__textarea_cursor_row_ptr(ta_ptr) : 0;
    int col = ta_ptr ? taro__textarea_cursor_col_ptr(ta_ptr) : 0;
...
static JSValue _js_viewport_scroll_up(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    const char* id = JS_ToCString(ctx, argv[0]);
    if (!id) return JS_EXCEPTION;
    void* vp = taro__viewport_get_ptr(id);
    JS_FreeCString(ctx, id);
    if (vp) taro__viewport_scroll_up_ptr(vp);
    return JS_UNDEFINED;
}
...
static JSValue _js_viewport_scroll_down(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    void* vp = taro__viewport_get_ptr(id);
    JS_FreeCString(ctx, id);
    if (vp) taro__viewport_scroll_down_ptr(vp);
    return JS_UNDEFINED;
}
...
static JSValue _js_viewport_page_up(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    void* vp = taro__viewport_get_ptr(id);
    JS_FreeCString(ctx, id);
    if (vp) taro__viewport_page_up_ptr(vp);
    return JS_UNDEFINED;
}
...
static JSValue _js_viewport_page_down(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    void* vp = taro__viewport_get_ptr(id);
    JS_FreeCString(ctx, id);
    if (vp) taro__viewport_page_down_ptr(vp);
    return JS_UNDEFINED;
}
...
void taro_register_async_globals(void* ctx) {
    if (!ctx) return;
    JSContext* c = (JSContext*)ctx;
    JSValue global = JS_GetGlobalObject(c);
    ...
    JS_SetPropertyStr(c, global, "__taro_log",    JS_NewCFunction(c, _js_log_stderr,     "__taro_log",    1));
